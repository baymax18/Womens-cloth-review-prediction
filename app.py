...
with tab2:
    ...
    with open(r"multinomial_nb_pipeline.pkl", 'rb') as file_1:
        mb_model = pickle.load(file_1)
        
    
    text = st.text_input("Enter your Review")

    predict_button = st.button(":green[Predict Ratings (Ranges from 1 to 5)]")

    if predict_button:
        if text.strip() == "":
            st.warning("Please enter a review before predicting.")
        else:         
            pred1 = mb_model.predict([text])[0]
            st.write(f"## :green[Predicted Rating : {pred1}]")